...
int main(int argc, char **argv){
    ...
    if (root == getpid()) {
        ...
        while (read(fd[0][0], &info, sizeof(struct Info)) > 0) {
            if (info.esNido) {
                printf("nido encontrado en la posicion [%d][%d] por el proceso %d\n", info.i, info.j, info.pid);
            }
        }

        while (read(fd[1][0], &info, sizeof(struct Info)) > 0) {
            if (info.esNido) {
                printf("nido encontrado en la posicion [%d][%d] por el proceso %d\n", info.i, info.j, info.pid);
            }
        }
...
bool hayNido(int **matriz, int i, int j, int row, int cols) {
    if(matriz[i][j] != 1) return false; // Si no es 1, no es una mina
    for (int vi = -1; vi <= 1; vi++) {
        for (int vj = -1; vj <= 1; vj++) {
            if(vi == 0 && vj == 0) continue; // Evitar la celda actual
            int ti = i + vi;
            int tj = j + vj;
            if((ti < 0 || ti >= row) || (tj < 0 || tj >= cols)) continue; // Evitar fuera de límites
            if(matriz[ti][tj] == 2) return true; // Si encuentra un 2, hay mina
        }
    }
    return false;
}
